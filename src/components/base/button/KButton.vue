...
const myProps = withDefaults(defineProps<Props>(), {})

const mySlots = defineSlots<ButtonSlots>()

const resolvedAttrs = computed(() => ({ ...useAttrs(), ...myProps, rounded: myProps.rounded ?? true }))

/**
 * @see https://tailwind.primevue.org/button/#presets.lara
 */
const preset = {
  root: ({ props, context, parent }: { props: Props, context: any, parent: any }) => ({
    class: clsx([
      ...
      // Alignments
      'inline-flex min-w-fit items-center justify-center text-center align-bottom',

      // Sizes & Spacing
      'leading-[normal]',
      {
        'h-10 px-4 py-4': props.size === null,
        'h-8 text-sm px-4 py-2': props.size === 'small',
        'h-12 text-lg px-4 py-3': props.size === 'large',
      },
      {
        'p-0 py-3': props.label == null && props.icon !== null,
      },
      ...
      // Secondary Button
      {
        ...
        'bg-secondary dark:bg-secondary-dark hover:bg-secondary-hover dark:hover:bg-secondary-dark-hover active:bg-secondary-pressed dark:active:bg-secondary-dark-pressed':
          props.severity === 'secondary' && !props.text && !props.outlined && !props.plain,
        'border-none':
          props.severity === 'secondary' && !props.text && !props.outlined && !props.plain,
      },
      ...
      // Tertiary button
      {
        'text-tertiary-text dark:text-secondary-dark-text':
          props.severity === 'tertiary' && !props.text && !props.outlined && !props.plain,
        'bg-transparent hover:bg-tertiary-hover dark:hover:bg-tertiary-dark-hover active:bg-tertiary-pressed dark:active:bg-tertiary-dark-pressed':
          props.severity === 'tertiary' && !props.text && !props.outlined && !props.plain,
        'border-none':
          props.severity === 'tertiary' && !props.text && !props.outlined && !props.plain,
      },
      ...
      // Primary
      {
        'hover:bg-primary-700 dark:hover:bg-primary-300 hover:border-primary-600 dark:hover:border-primary-300':
          !props.link && props.severity === null && !props.text && !props.outlined && !props.plain,
      },
      { 'focus:ring-bg-primary-70 dark:focus:ring-primary-300/50': props.severity === null },
      // Text & Outlined Button
      { 'hover:bg-btn-hover': (props.text || props.outlined) && props.severity === null && !props.plain },

      // Secondary
      // {
      //   'hover:bg-surface-300 dark:hover:bg-surface-300 hover:border-surface-600 dark:hover:border-surface-300':
      //     props.severity === 'secondary' && !props.text && !props.outlined && !props.plain,
      // },
